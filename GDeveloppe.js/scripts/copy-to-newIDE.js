...
const sourcePath = path.join(__dirname, '../../Binaries/embuild/GDeveloppe.js');
...
const copyLibGdJsFile = (filePath) => {
  if (
    ...
    shell.echo(
      `✅ Copied ${path.basename(filePath)} from Binaries/embuild/GDeveloppe.js to public and node_modules folders of newIDE/app.`
    );
  } else {
    shell.echo(
      `❌ Error while copying ${path.basename(filePath)} from Binaries/embuild/GDeveloppe.js to public and node_modules folders of newIDE/app.`
    );
  }
};
...
if (!shell.test('-f', path.join(sourcePath, 'libGD.js'))) {
  shell.echo('❌ You must compile GDeveloppe.js first');
  shell.exit(1);
}
...
if (
  ...
  shell.echo(
    '✅ Copied libGD.js from Binaries/embuild/GDeveloppe.js to public and node_modules folders of newIDE/app.'
  );
} else {
  shell.echo(
    '❌ Error while copying libGD.js from Binaries/embuild/GDeveloppe.js to public and node_modules folders of newIDE/app'
  );
  shell.exit(1);
}
