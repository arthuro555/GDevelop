...
import { SearchInEventsInputs, ReplaceInEventsInputs } from './EventsSearcher';
...
type Props = {
  ...
  onGoToPreviousSearchResult: () => gd.BaseEvent | null | undefined;
  onGoToNextSearchResult: () => gd.BaseEvent | null | undefined;
...
const SearchPanel = (
  ...
  ref:
    | ((arg1: null | SearchPanelInterface) => unknown)
    | {
        current: null | SearchPanelInterface;
      }
) => {
  const { isMobile } = useResponsiveWindowSize();
  const searchTextField = React.useRef<TextFieldInterface | null | undefined>(
    null
  );
  const replaceTextField = React.useRef<TextFieldInterface | null | undefined>(
    null
  );
...
export default React.forwardRef<SearchPanelInterface, Props>(SearchPanel);
