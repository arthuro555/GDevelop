...
export const downloadUrlFilesToBlobFiles = async (
  ...
  const downloadedBlobs: Array<ItemResult<UrlFileDescriptor>> = await downloadUrlsToBlobs({
...
export const archiveFiles = async (
  ...
  return new Promise((resolve: (result: Promise<Blob> | Blob) => void, reject: (error?: any) => void) => {
    zipJs.createWriter(
