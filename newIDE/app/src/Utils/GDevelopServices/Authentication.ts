...
import { CommunityLinks, UserSurvey } from './User';
...
export type EditForm = {
  ...
  readonly getGameStatsEmail: boolean,
  readonly getNewsletterEmail: boolean,
  readonly donateLink: string,
  readonly discordUsername: string,
  readonly communityLinks: CommunityLinks
};
...
export type AuthError = {
  code: 'auth/invalid-email' | 'auth/missing-email' | 'auth/user-disabled' | 'auth/user-not-found' | 'auth/wrong-password' | 'auth/email-already-in-use' | 'auth/operation-not-allowed' | 'auth/weak-password' | 'auth/username-used' | 'auth/malformed-username' | 'auth/requires-recent-login' | 'auth/too-many-requests' | 'auth/internal-error' | 'auth/network-request-failed' | 'auth/account-exists-with-different-credential'
};
...
export default class Authentication {
  ...
  getFirebaseUser = async (): Promise<FirebaseUser | null | undefined> => {
    const { currentUser } = this.auth;
    if (!currentUser) {
      return null;
    }
  ...
  getUserProfile = async (getAuthorizationHeader: () => Promise<string>): Promise<Profile> => {
    const { currentUser } = this.auth;
    if (!currentUser)
  ...
  getFirebaseUserSync = (): FirebaseUser | null | undefined => {
