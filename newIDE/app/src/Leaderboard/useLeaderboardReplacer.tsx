...
export const useLeaderboardReplacer = (): UseLeaderboardReplacerOutput => {
  const [project, setProject] = React.useState<gd.Project | null>(null);
  const [gameId, setGameId] = React.useState<string | null>(null);
  const [shouldReplace, setShouldReplace] = React.useState(false);
  const [leaderboardsToReplace, setLeaderboardsToReplace] = React.useState<
   ...
   */
  const ensureLeaderboardsAreReplaced = React.useCallback(async () => {
    ...
    const duplicateLeaderboardAndStepProgress = async (
      ...
      leaderboardId: string
    ): Promise<ErroredLeaderboard | null | undefined> => {
      try {
  ...
  const findLeaderboardsToReplace = React.useCallback(
    (project: gd.Project, sourceGameId: string) => {
      setProject(project);
