...
// @ts-expect-error - TS7031 - Binding element 'item' implicitly has an 'any' type. | TS7031 - Binding element 'buildFromTemplate' implicitly has an 'any' type.
...
// @ts-expect-error - TS1005 - ',' expected. | TS7005 - Variable 'any' implicitly has an 'any' type. | TS1005 - ';' expected.
  ...
  const handlePointerOver = (pointerEvent: React.PointerEvent) => {
    if (item.enabled === false) {
      return;
    }
  ...
  function handleLeave(pointerEvent: React.PointerEvent) {
    // If not a mouse, do nothing: all menus are opened/closed
    // by clicking on them (`handleClick`) or outside (`handleClose` called when the backdrop is clicked).
    // There is no notion of the "cursor" for touch/pen leaving a menu.
    if (pointerEvent.pointerType !== 'mouse') {
