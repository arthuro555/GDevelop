...
type AssetStoreState = {
  ...
  privateAssetPackListingDatasSearchResults: Array<PrivateAssetPackListingData> | null | undefined,
...
export const AssetStoreContext = React.createContext<AssetStoreState>(initialAssetStoreState);
...
export const AssetStoreStateProvider = ({
  ...
  ] = React.useState<Array<AssetShortHeader> | null | undefined>(null);
  ...
  const [
    publicAssetPacks,
    setPublicAssetPacks,
  ] = React.useState<PublicAssetPacks | null | undefined>(null);
  const [
...
// @ts-expect-error - TS2339 - Property 'showAlert' does not exist on type 'void'.
  ...
  const [
    animatedFilter,
    setAnimatedFilter,
  ] = React.useState<AnimatedAssetStoreSearchFilter>(new AnimatedAssetStoreSearchFilter());
  const [
    viewpointFilter,
    setViewpointFilter,
  ...
  const [
    dimensionFilter,
    setDimensionFilter,
  ] = React.useState<DimensionAssetStoreSearchFilter>(new DimensionAssetStoreSearchFilter());
  const [
    objectTypeFilter,
    setObjectTypeFilter,
  ] = React.useState<ObjectTypeAssetStoreSearchFilter>(new ObjectTypeAssetStoreSearchFilter());
  const [
    colorFilter,
    setColorFilter,
  ] = React.useState<ColorAssetStoreSearchFilter>(new ColorAssetStoreSearchFilter());
  const [
    licenseFilter,
    setLicenseFilter,
  ] = React.useState<LicenseAssetStoreSearchFilter>(new LicenseAssetStoreSearchFilter());
  // When one of the filter change, we need to rebuild the array
  // for the search.
  const assetSearchFilters = React.useMemo<Array<SearchFilter<AssetShortHeader>>>(() => [
    animatedFilter,
    viewpointFilter,
    dimensionFilter,
    objectTypeFilter,
  ...
  ] = React.useState<AssetPackTypeStoreSearchFilter>(new AssetPackTypeStoreSearchFilter({ receivedAssetPacks }));
  const assetPackSearchFilters = React.useMemo<Array<SearchFilter<PublicAssetPack | PrivateAssetPackListingData>>>(() => [assetPackTypeFilter], [assetPackTypeFilter]);
  ...
  const assetShortHeadersSearchResults: Array<AssetShortHeader> | null | undefined = useSearchItem(
  ...
  const publicAssetPacksSearchResults: Array<PublicAssetPack> | null | undefined = useSearchItem(
  ...
  const privateAssetPackListingDatasSearchResults: Array<PrivateAssetPackListingData> | null | undefined = useSearchItem(
