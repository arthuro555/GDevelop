...
const useGamesList = () => {
  ...
  const gamesFetchingPromise = React.useRef<Promise<any> | null | undefined>(null);
  const {
    authenticated,
    firebaseUser,
  ...
  const [games, setGames] = React.useState<Array<Game> | null | undefined>(null);
  const [gamesFetchingError, setGamesFetchingError] = React.useState<Error | null | undefined>(null);
