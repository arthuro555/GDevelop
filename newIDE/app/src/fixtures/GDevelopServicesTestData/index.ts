...
import { Comment } from '../../Utils/GDevelopServices/Play';
import {
  Announcement,
  Promotion,
...
// @ts-expect-error - TS2740 - Type '{ uid: string; providerId: string; email: string; emailVerified: true; }' is missing the following properties from type 'User': isAnonymous, metadata, providerData, refreshToken, and 9 more.
...
const generateGameRollingMetricsFor364Days = () => {
  ...
  const metrics: Array<GameMetrics> = [];
  const count = 364;
