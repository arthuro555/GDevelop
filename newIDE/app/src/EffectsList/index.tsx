  ...
  (
    {
      ...
      removeEffect: (effect: gd.Effect) => void;
      copyEffect: (effect: gd.Effect) => void;
      pasteEffectsBefore: (effect: gd.Effect) => Promise<void>;
      chooseEffectType: (effect: gd.Effect, newEffectType: string) => void;
      allEffectMetadata: Array<EnumeratedEffectMetadata>;
      nameErrors: {
        [key: number]: React.ReactNode;
      };
      setNameErrors: (nameErrors: { [key: number]: React.ReactNode }) => void;
    ...
    const renameEffect = React.useCallback(
      (effect: gd.Effect, newName: string) => {
...
const getEnumeratedEffectMetadata = (
  ...
  effectType: string
): EnumeratedEffectMetadata | null | undefined => {
...
export default function EffectsList(props: Props) {
  ...
  const draggedEffect = React.useRef<gd.Effect | null | undefined>(null);
  ...
  const chooseEffectType = React.useCallback(
    (effect: gd.Effect, newEffectType: string) => {
  ...
  const copyAllEffects = React.useCallback(() => {
    Clipboard.set(
      ...
      mapFor(0, effectsContainer.getEffectsCount(), (index: number) => {
        const effect: gd.Effect = effectsContainer.getEffectAt(index);
        return {
          name: effect.getName(),
          type: effect.getEffectType(),
          serializedEffect: serializeToJSObject(effect),
