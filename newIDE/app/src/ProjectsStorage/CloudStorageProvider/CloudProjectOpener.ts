...
// @ts-expect-error - TS7034 - Variable 'projectCache' implicitly has type 'any' in some locations where its type cannot be determined.
let projectCache;

export const getProjectCache = () => {
// @ts-expect-error - TS7005 - Variable 'projectCache' implicitly has an 'any' type. | TS7005 - Variable 'projectCache' implicitly has an 'any' type.
...
export const generateGetAutoSaveCreationDate = (
  ...
  ProjectCache.isAvailable()
    ? async (fileMetadata: FileMetadata, compareLastModified: boolean): Promise<number | null | undefined> => {
        const { profile } = authenticatedUser;
