...
export type FetchAllProjectResourcesFunction = (options: FetchAllProjectResourcesOptions) => Promise<FetchAllProjectResourcesResult>;
...
type UseResourceFetcherOutput = {
   ...
   */
  ensureResourcesAreFetched: (getOptions: () => EnsureResourcesAreFetchedOptions) => Promise<void>,
  /**
...
export const useResourceFetcher = (
  ...
  const {
    ensureProcessIsDone,
    renderProcessDialog,
  } = useGenericRetryableProcessWithProgress<() => EnsureResourcesAreFetchedOptions>({
    onDoProcess: React.useCallback(
// @ts-expect-error - TS7006 - Parameter 'getOptions' implicitly has an 'any' type. | TS7006 - Parameter 'onProgress' implicitly has an 'any' type.
