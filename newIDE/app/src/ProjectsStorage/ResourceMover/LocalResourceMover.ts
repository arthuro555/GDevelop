...
export const moveAllLocalResourcesToCloudResources = async ({
   ...
   */
  const getResourcesToUpload = (project: gd.Project): Array<gd.Resource> => {
    const allResourceNames = resourcesManager.getAllResourceNames().toJSArray();
    return allResourceNames
      .map((resourceName: string): gd.Resource | null | undefined => {
  ...
  [`${LocalFileStorageProvider.internalName}=>${LocalFileStorageProvider.internalName}`]:
    async ({ project, newFileMetadata }: MoveAllProjectResourcesOptions) => {
      ...
      gd.ProjectResourcesCopier.copyAllResourcesTo(
        project,
        // $FlowFixMe - fileSystem is a gd.AbstractFileSystem, despite the assignIn.
        fileSystem,
        projectPath,
